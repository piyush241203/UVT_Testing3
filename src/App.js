...
function RelativeTime({ base }) {
    ...
    useEffect(() => {
        const update = () => {
            ...
            if (diff < 60)
                setLabel(`${diff} seconds ago`);
            else if (diff < 3600)
                setLabel(`${Math.floor(diff / 60)} minutes ago`);
            else
                setLabel(`${Math.floor(diff / 3600)} hours ago`);
        };
        update();
    ...
    return (<div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <h2 style={{ margin: 0, color: '#10b981', fontWeight: '800', letterSpacing: '-0.5px' }}>✨ UVT React Demo v3.2 (Remote CI Final Phase)</h2>
        </div>
...
function About() {
    return (<div>
      ...
      <div style={{ display: 'flex', gap: '16px', marginTop: '16px', flexWrap: 'wrap' }}>
        {['Alice', 'Bob', 'Carol'].map(name => (<div key={name} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
...
function FAQ() {
    ...
    return (<div>
      ...
      {items.map((item, i) => (<div key={i} style={{ marginBottom: '8px', border: '1px solid #eee', borderRadius: '8px', overflow: 'hidden' }}>
          <button onClick={() => setOpen(open === i ? null : i)} style={{ width: '100%', textAlign: 'left', padding: '12px 16px', background: '#f9f9f9', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
            {item.q}
          </button>
...
function Team() {
    ...
    return (<div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
        {members.map(m => (<div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', border: '1px solid #cbd5e1', borderRadius: '12px', background: '#f8fafc' }}>
...
function Careers() {
    ...
    return (<div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {jobs.map(j => (<div key={j} style={{ padding: '16px', border: '1px solid #e2e8f0', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: '#f5f5f5' }}>
            {['ID', 'Item', 'Status', 'Amount', 'Date'].map(h => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', borderBottom: '2px solid #eee' }}>{h}</th>))}
          </tr>
        </thead>
        <tbody>
...
function Admin() {
    return (<div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
        {['Users', 'Tokens', 'Webhooks', 'Audit Logs'].map(section => (<div key={section} style={{ padding: '16px', border: '1px solid #eee', borderRadius: '8px' }}>
...
function Search() {
    ...
    return (<div>
      ...
      {results.length > 0 ? (<ul>{results.map(r => <li key={r}><Link to={`/${r.toLowerCase()}`}>{r}</Link></li>)}</ul>) : query.length >= 2 ? (<p>No results for "<strong>{query}</strong>"</p>) : null}
    </div>);
}
function ErrorPage() {
...
function Contact() {
    ...
    return (<div>
      ...
      {submitted ? (<div style={{ color: 'green', fontWeight: 'bold' }}>Message sent! Thank you. <RelativeTime base={new Date()}/></div>) : (<form onSubmit={e => { e.preventDefault(); setSubmitted(true); }} style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '360px' }}>
...
function Dashboard() {
    ...
    return (<div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
        {Object.entries(data.metrics).map(([k, v]) => (<div key={k} style={{ padding: '14px', background: '#fff7ed', borderRadius: '10px', textAlign: 'center', border: '1px solid #fed7aa' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {data.items.map((it) => (<tr key={it.id} style={{ borderBottom: '1px solid #eee' }}>
...
function ChartMock() {
    ...
    return (<div style={{ padding: '16px', background: '#fff', border: '1px solid #ddd', borderRadius: '8px', marginBottom: '16px' }}>
...
function DataTable() {
    ...
    return (<table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px' }}>
      <thead>
...
function Accordion() {
    ...
    return (<div style={{ border: '1px solid #ddd', borderRadius: '8px', marginBottom: '16px', overflow: 'hidden' }}>
      {['Overview', 'Advanced Settings', 'Danger Zone'].map((title, idx) => (<div key={title} style={{ borderBottom: idx === 2 ? 'none' : '1px solid #ddd' }}>
          <div onClick={() => setOpenIdx(openIdx === idx ? null : idx)} style={{ padding: '16px', background: '#f8fafc', cursor: 'pointer', fontWeight: 600, display: 'flex', justifyContent: 'space-between' }}>
            {title}
            <span>{openIdx === idx ? '▲' : '▼'}</span>
          </div>
          {openIdx === idx && (<div style={{ padding: '16px', background: '#fff' }}>
...
function Tabs() {
    ...
    return (<div style={{ marginBottom: '16px' }}>
      <div style={{ display: 'flex', borderBottom: '1px solid #ddd' }}>
        {['data', 'chart', 'logs'].map(t => (<button key={t} onClick={() => setTab(t)} style={{ padding: '12px 24px', border: 'none', background: tab === t ? '#fff' : 'transparent', borderBottom: tab === t ? '2px solid #3b82f6' : 'none', cursor: 'pointer', textTransform: 'capitalize', fontWeight: tab === t ? 600 : 400 }}>
            {t}
          </button>))}
      </div>
...
function Billing() {
    return (<div className="space-y-6">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
        ...
        ].map(p => (<div key={p.plan} style={{ padding: '24px', border: p.current ? '2px solid #10b981' : '1px solid #ddd', borderRadius: '12px', textAlign: 'center' }}>
